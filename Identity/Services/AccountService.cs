...
namespace Identity.Services
{
    public class AccountService : IAccountService
    {
        ...
        public AccountService(UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, JWTSettings jWTSettings, IDateTimeService dateTimeService, RoleManager<IdentityRole> roleManager)
        {
        ...
        public async Task<Response<AuthenticationResponse>> AuthenticateAsaync(AuthenticationRequest request, string ipAddress)
        {
            var usuario = await _userManager.FindByEmailAsync(request.Email);
            if (usuario == null)
            {
                throw new ApiException($"No hay una cuenta registrada con email ${request.Email} .");
            }
        ...
        {
            using var rngCryptoServicceProvider = new RNGCryptoServiceProvider();
            var randomBytes = new byte[40];
            rngCryptoServicceProvider.GetBytes(randomBytes);
